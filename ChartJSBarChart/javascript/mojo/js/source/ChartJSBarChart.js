...
(function () {
    ...
    mstrmojo.plugins.ChartJSBarChart.ChartJSBarChart = mstrmojo.declare(
        mstrmojo.CustomVisBase,
        null,
        {
            ...
            externalLibraries: [{
				url : "//code.jquery.com/jquery-3.2.1.min.js"
			},
			{
            ...
            plot: function () {
				...
				try{
					...
					if(validationFlag){
						...
						var fillColor = pickColorForBar(element);
						if(xDataList != undefined && !jQuery.isEmptyObject(xDataList) && yDataList != undefined && !jQuery.isEmptyObject(yDataList)){
							renderHtml(xDataList,yDataList,xAxisLabel,graphTitle,formattedValueList,dataJson.length,fillColor); 	
						}else{
							$("#barChartError").append("Insuffiecient data.");	 
						}
					}else{
						$("#barChartError").append("Grid Structure is not appropriate. Please refer README file.");
					}		
				}catch(err){
					console.log("Error : plot function");
					$("#barChartError").append("Something went wrong.");
				}
				
				function renderHtml(xDataList, yDataList, xAxisLabel, graphTitle, formattedValueList, sizeOfData,fillColor) {
							debugger;
