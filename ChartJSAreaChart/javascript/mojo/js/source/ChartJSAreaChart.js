 ...
 */
(function () {
    if (!mstrmojo.plugins.ChartJSAreaChart) {
        mstrmojo.plugins.ChartJSAreaChart = {};
    }
    ...
    mstrmojo.plugins.ChartJSAreaChart.ChartJSAreaChart = mstrmojo.declare(
        mstrmojo.CustomVisBase,
        null,
        {
            scriptClass: "mstrmojo.plugins.ChartJSAreaChart.ChartJSAreaChart",
            cssClass: "ChartJSAreaChart",
            errorMessage: "Either there is not enough data to display the visualization or the visualization configuration is incomplete.",
            errorDetails: "This visualization requires one or more attributes and one metric.",
            externalLibraries: [{
				url : "../plugins/ChartJSAreaChart/javascript/jquery.min.js"
			},
			{
            ...
            plot: function () {
				...
				var errorDiv = '<div id="areaChartError" style="color:red;font-size: 10pt;"></div>';
				...
				try{
					...
					if(validationFlag){
						...
						if(xDataList != undefined && !jQuery.isEmptyObject(xDataList) && yDataList != undefined && !jQuery.isEmptyObject(yDataList)){
							...
							$("#areaChartError").append("Insuffiecient data.");	 
						}
					}else{
						$("#areaChartError").append("Grid structure is not appropriate. Please refer README file.");
					}		
				}catch(err){
					console.log(err);
					$("#areaChartError").append("Something went wrong. Open console to identify an error.");
				}
